...
###특별한 정보
    ...
    Host(필수) : 요청한 호스트 정보(도메인), 하나의 서버에서 여러개의 어플리케이션을 사용할 때 구분값으로 사용
    Location : 웹브라우저 3xx 응답의 결과에 Location 헤더가 있으면 페이지 자동 이동
                201(created) Location 값은 요청에 의해 생성된 소스 URI
    Allow : 허용 가능한 HTTP 메서드, 실제서버에서 적용되어있는 경우 별로 없음
    Retry-after : 유저 에이전트가 다음 요청을 하기까지 기다려야 하는 시(날짜 단위, 초 단위)
    
###인증

    클라이언트 인증 정보를 서버로 전달
    인증 실패 시 401 Unauthorized응답에 요구사항을 정의해서 클라이언트에 전달

###쿠키

    HTTP 통신은 클라이언트가 보낸 요청을 서버가 받아서 응답한 후에 서로의 연결을 끊는다
    로그인 같은 기능이 있는 홈페이지는 사용자를 구분하기 위해 특정 상태값을 지속적으로 유지 해야 하는데
    이러한 문제를 해결하기 위해 쿠키라는 기술을 도입
    
    Set-Cookie : 서버에서 클라이언트로 쿠키 전(응답)
    Cookie : 클라이언트가 서버에서 받은 쿠키를 저장, HTTP 요청 시 서버로 전달

    Cookie 플로우
    1.큻라이언트에서 서버에 사용자 정보를 전달
    2.서버에서 사용자정보를 확인하고 Set-Cookie에 값을 설정하여 클라이언트에 전달
    3.클라이언트에 쿠키저장소가 있으며 Set-Cookie에 넘어온 값을 보관하고 요청시 쿠키저장소를 확인하여 요청에 쿠키도 같이 전달
    4.3번째 행위 이후 별도의 사용자 인증없이 사용자 인증객체를 유지하면서 사용

    Set-Cookie 에 값으로 사용자 정보를 담을 수 있지만 보안상 sessionId 같은 추측 불가능한 값으로 대체하여 사용

    Expires : 날짜로 설정 값을 주어 쿠키 만료 ex) Sat, 26-Dec-2020 04:39:21 GMT
    max-age : 초 단위로 설정 값을 주어 쿠키 만료 ex) 3600 = 3600초
    세션 쿠키 : 만료 날짜를 생략하면 브라우저 종료 시 까지 유지
    영속 쿠키 : 만료 날짜를 입력하여 해당 날짜까지 유지

    쿠키-도메인 : 명시한 문서 기준 도메인 + 서브 도메인 포함
                domain=example.org 로 설정하면 example.org를 포함한 상위 도메인도 접근 가능
                domain 값을 생략하면 해당 도메인에만 쿠키 접근 나머지는 불가능
    쿠키-경로 : 해당 경로를 포함한 하위 페이지에만 쿠키 접근 가능

    secure : 쿠키는 http, https를 구분하지 않고 전송하는데 설정 값으로 secure를 설정하면 https인 경우 쿠키 전송
    httpOnly : XSS 공격 방지, javascript에서 쿠키를 접근할 수 있는데 httpOnly 설정 시 Http 전송에서만 사용 가능
    sameSite : XSRF 공격 방지, 요청 도메인과 쿠키에 설정된 도메인이 같은 경우만 쿠키 전송

###HTTP 헤더 요청 (캐시와 조건부 요청)

    웹브라우저에는 브라우저 캐시라는 저장소가 있으며 cache-control:max-age=?? 로 특정시간동안 보관할 수 있다
    서버에 요청하여 응답받을 때 헤더에 last-modified 값을 설정하여 받으며

    캐시와 조건부 요청 플로우 (Last-Modified, if-modified-since)
    1.서버가 클라이언트 요청에 대한 응답 값으로 Last-Modified 값 전달
    2.클라이언트가 서버에 다시 요청할 때 if-modified-since 에 응답 받은 Last-Modified 값을 전달
    3.서버에서 if-modified-since 값으로 변경을 확인할 수 있으며 변경되지 않았다는 의미로 304 Not Modified 와 바디 없이 응답
    4.응답에 헤더 값 만 있어 네트워크 부하가 줄어듬, 응답으로 304코드를 받고 클라이언트의 저장소에서 요청에 대한 응답 해결
    
    캐시와 조건부 요청 플로우 (ETag, If-None-Match)
    Last-Modified와 플로우는 동일하면서 ETag라는 컬럼에 특정 구분 값(ex)해쉬값)을 사용하여 데이터의 변경 상태 확인 

###캐시와 조건부 요청

    Cache-Control : max-age : 캐시 유효 시, 초 단위
    Cache-Control : no-cache : 데이터는 캐시 해도 되지만 항상 원(origin)서버에 검증하고 사용
        캐시를 검증하는 중간서버들이 있는데 그런 서버들은 지나치고 실제 응답하는 서버에서 검증하고 응답
    Cache-Control : no-store : 데이터에 민감한 정보가 있을 경우 저장하지 않도록 지시

    캐시 지시어 : 확실한 캐시 무효화
        Cache-Control : no-cache, no-store, must-revalidate
    must-revalidate : 캐시 만료후 최초 조회 시 원 서버에서 검증 필요
    no-cache의 경우 중간에 네트워크 문제 발생시 기존 데이터를 보여줄 수 있지만 must-revalidate는 무조건 에러 페이지를 응답